...
<table id='main' border='0' cellspacing='4'>
	<tr>
		<td align='center' valign='top' class='left_right_td_border'>
			<?php e($this->element('left', array('action'=>'inbox'))); ?>
		</td>
		<td align='center' valign='top' class='center_td_border'>
			<?php if(count($whispers)<1){ ?>
				<div class='new_whisper_box'>
					<div class='general_title' align='center'>Opps! Nobody has ever whispered to you yet.</div>
				</div>
			<? } ?>			
			<?php for($i=0; $i<count($whispers); $i++){ ?>
				...
				<? }else{ ?> <div class='read_item_in_whisper_list'> <? } ?>
					<table width='100%' border='0'>
						<tr>
							<td width='70%'>
								<?php echo $html->link($creators[$whispers[$i]["Whisper"]["id"]][0]["users"]["firstname"].' '.$creators[$whispers[$i]["Whisper"]["id"]][0]["users"]["secondname"], '/users/profile/'.$creators[$whispers[$i]["Whisper"]["id"]][0]["users"]["id"]).' <span class="mini_instructions">whispered to you</span>'; ?> : <?php echo '<span class="whisper_summary">'.$html->link($this->Presentation->shortenString($whispers[$i]['Whisper']['content'], 25).' ...', '/whispers/view/'.$whispers[$i]['Whisper']['id']).'</span>'; ?>
							</td>
							<td align='right'>
